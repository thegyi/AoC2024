...
void tryToCorrect(std::vector<int> &numbers,
                  std::map<int, std::vector<int>> &rules) {
  for (int i = 1; i < numbers.size(); i++) {
    for (int j = 0; j < i; j++) {
      auto pos = std::ranges::find(rules[numbers[i]], numbers[j]);
      if (pos != rules[numbers[i]].end()) {
...
int main() {
  ...
  while (std::getline(ser, line)) {
    ...
    if (!seriesIsCorrect(numbers, rules)) {
      while (!seriesIsCorrect(numbers, rules)) {
        tryToCorrect(numbers, rules);
      }
